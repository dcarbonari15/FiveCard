    ...
    public void playerDiscards(int sourcePlayer, Card[] card){
        if (card.length > 0) {
            ...
            for (int i = 0; i < card.length; i++) {
                ...
                for (int j = 0; j < lobby.get(sourcePlayer).getHand().length; j++) {
                    //check if the two cards match
                    if (card[i].getRank() == lobby.get(sourcePlayer).getCard(j).getRank()
                            && card[i].getSuit() == lobby.get(sourcePlayer).getCard(j).getSuit()) {

                        //set the discarded card to the top card on the deck
                        lobby.get(sourcePlayer).setCard(j, deck.get(0));
                        //remove top card
                        deck.remove(0);
                    }
                }
    ...
    public void dealCards() {
        ...
        for (int i = 0; i < 5; i++){
            ...
            for (int j = 0; j < lobby.size(); j++) {
                lobby.get(j).setCard(i, deck.get(0));
                dealtCards.add(deck.get(0));
                deck.remove(0);

                if ((GamePlayer)lobby.get(j) instanceof FCDHumanPlayer){
                    ((FCDHumanPlayer)((GamePlayer)(lobby.get(j)))).setCardImage(i, deck.get(0));
                }
            }
        }
    }
